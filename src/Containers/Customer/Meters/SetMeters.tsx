...
import {getCustomerMeters, updateUsage} from "../../../DBHandler/CustomerFunctions"
...
const SetMeters = () => {
  ...
  const submitUsage = (e:React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    e.preventDefault()
  ...
  return (
    <div>
      <h2>
        {showModal ? (error ? "Error in making change" : "Usage changed successfully") : null}
      </h2>
      <table>
        <thead>
          <tr>
          ...
          {meterData.map((row, index) => {
            return(
              <tr key={index} >
                ...
                <td>{row[2]}</td>
                <td>
                  <input type="number" name="usage-input" id="" 
      ...
      <button 
      onClick={(e) => submitUsage(e)} > Set Values </button>
    </div>
  );
};
